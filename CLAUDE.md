...
2. **Service Layer** (`src/services/{domain}Service.js`):
   ...
   - File uploads use `multipart/form-data` via the same axios instance (e.g. `productService.uploadProductImage(id, file)` builds a `FormData` with a `file` part and `POST`s to `/{id}/image` — do **not** hand-set `Content-Type`, the browser adds the multipart boundary; the JWT interceptor still applies). Returns the updated DTO (incl. `imageUrl`)
...
- **Sale register view (`/sale/register`) breaks the table/form pattern** — it is an asymmetric POS layout, not a CRUD form. `SaleForm.jsx` composes `useSaleForm` and renders a MUI Grid v2 split: left ~70% = `ProductCatalog.jsx` (search + responsive grid of `ProductCard.jsx` tiles, replaced the old products table) and right ~30% (see perf note below) = a fixed "ticket" panel (`OperationTypeToggle.jsx` admin-only + `SaleInfo.jsx` + conditional `DeliveryOrder.jsx` + `ShoppingCart.jsx`). `SaleInfo`'s green header carries an **"Abrir Caja"** `IconButton` (`PointOfSale` icon, next to the user chip) rendered only when `canOpenDrawer` (ADMIN or CAJERO — backend rejects PEDIDOS); it calls `useSaleForm.handleOpenDrawer` to pop the cash drawer without a sale (see Local Agent). `ShoppingCart` is a flex column whose totals + "Cobrar Venta"/"Cancelar" footer stays pinned while the cart list scrolls. Clicking a `ProductCard` opens `AddProductForm.jsx` (quantity + scale via `QuantityInput`, read-only price/total). `ProductCard` shows the product photo via `<Avatar src={product.imageUrl}>` — MUI's native fallback renders the initials+`categoryColor` placeholder when `imageUrl` is null/empty/broken, so the `productInitials`/`categoryColor` helpers stay the fallback (no extra state, `React.memo` intact). Reusable `sx` and the per-category color/label/initials helpers for this view live in `src/styles/js/SaleFormStyles.js`. **Post-sale flow:** "Cobrar Venta" opens `PaymentModal.jsx` (numpad/denominations + "Requiere Factura"); on a successful save `SaleForm` renders `PostSaleSummaryModal.jsx` (see Ticket/Printing) — there is **no** "print ticket" checkbox in the payment modal.
  - **Catalog performance pattern (preserve this):** the product list can be large (~250 items), so `ProductCatalog` paginates client-side at `ITEMS_PER_PAGE = 24` (slices `filteredProducts` before mapping; MUI `<Pagination>` below the grid; `page` resets to 1 via `useEffect` on `productSearch`; grid breakpoints `{xs:6, sm:4, md:3, lg:3, xl:2}` = 2/3/4/4/6 tiles per row). `ProductCard` is wrapped in `React.memo`, kept effective by a referentially stable `onSelect` chain: `useSaleForm.handleSelectProduct` (memoized with `useCallback`) → `SaleForm.handleOpenProductDialog` (`useCallback`) → `ProductCatalog.handleSelect` (`useCallback`). Keep these callbacks stable when editing this view.
- **Quick price-edit view (`/products/quick-prices`, `AdminRoute`, admin post-login landing) also breaks the table pattern** — mobile-first touch UI, not a DataGrid. `ProductPriceEditor.jsx` (composes `useQuickPrices`) renders a responsive MUI Grid v2 (`size={{xs:12, sm:6, md:4, lg:3}}`) of memoized `ProductPriceCard.jsx` tiles, inspired by `ProductCatalog`: sticky search with **300ms debounce** (private `useDebounce` in the hook), client-side `<Pagination>` (`ITEMS_PER_PAGE = 24`). Each card has a `type="number" inputMode="decimal"` price `TextField` with a dynamic `aria-label` ("Precio de {name}") and saves **both** onBlur and via a ≥48px Save button (touch target ≥44px). Reuses the existing data layer — `productService.updateProductPrice` (`PUT /update-price/{id}`, body `{price}`) + `useProduct.handleUpdatePriceProduct` (already routed through `useApiErrorHandler`); **no new API/service/slice**. `useQuickPrices.handleSavePrice` is `useCallback`-stable to keep `ProductPriceCard`'s `React.memo` effective. Each card's `<Avatar src={product.imageUrl}>` shows the product photo with the same native initials+`categoryColor` fallback as `ProductCard`. Reuses `productInitials`/`categoryColor`/`categoryLabel` from `SaleFormStyles.js`; own `sx` in `src/styles/js/QuickPricesStyles.js`. This view **replaced the retired** `SimpleProductTable` + `useProductTableSimple` (old `/product/price-change` route, now removed).
...
- Special hooks:
  - `useSaleForm`: Complex form hook managing cart state, customer discounts, product search, validation, delivery order integration, and **hot-stamp invoicing** (composes `useCustomerFiscalData` for the fiscal-profile list and `useInvoice` to call `handleTimbrarInvoice` after a successful save, then auto-fires `handleSendInvoiceEmail` fire-and-forget if `fiscalData.emailFacturacion` is populated — email failure never blocks the committed sale). Supports two operation types: `'venta'` (sale) and `'pedido'` (delivery order). Exposes `requiresInvoice`, `selectedFiscalId`, `customerFiscalDataList`, and their setters alongside the cart/customer state. **Deferred-print / post-sale summary:** the sale payload always sends `printTicket: false` (backend never auto-prints on save). On a successful save `handleSubmit` does **not** clear the form immediately — it fills `postSaleData` (`{isOpen, saleId, total, amountTendered, changeDue}`) to open `PostSaleSummaryModal`. `handlePrintPostSaleTicket` (memoized) calls `getTicketBySaleId(saleId, getCachedStationId())` to trigger the backend's print side-effect, then runs cleanup; `handleClosePostSaleModal` (memoized) just runs cleanup. The `shouldPrintTicket` checkbox approach was removed. Also exposes `handleOpenDrawer` (memoized) + `canOpenDrawer` (ADMIN/CAJERO) for the manual cash-drawer button — calls `saleService.openDrawer(getCachedStationId())`, toasts `"Comando enviado"` on success, routes failures through `useApiErrorHandler`.
...
- **Deferred backend print (post-sale):** sales are saved with `printTicket: false`, so the backend (`POST /save`) does **not** auto-print. Instead the `GET /api/sales/{saleId}/ticket?stationId=...` endpoint has a side-effect that sends the ticket to the station's printer over WebSocket. `PostSaleSummaryModal.jsx` (MUI `Dialog`, opened by `SaleForm` after a successful sale) shows the change to return (`Typography variant="h3"`) plus total + cash tendered, with two actions: **"Imprimir Ticket"** → `useSaleForm.handlePrintPostSaleTicket` (fires the ticket GET to print, then cleans up the cart) and **"Cerrar / No Imprimir"** → `handleClosePostSaleModal` (cleanup only, no print).
...
- `src/utils/formatters.js`: Localized formatting functions
  ...
  - `toNumber(value)`: safe string/number → finite number (handles `,` decimal separator, null/empty → 0)
  - `formatQuantity(value)`: display-normalizes a quantity to max 3 decimals, trailing zeros stripped (used by `ShoppingCart`'s "Cant." cell)
- **Decimal precision (money/weight):** `useSaleForm.js` has private `formatToTwoDecimals` / `formatToThreeDecimals` helpers using `Math.round((n + Number.EPSILON) * factor) / factor` (factor 100 / 1000) — do **not** revert to string-exponent tricks (`"e2"`), they break on exponential-notation inputs. All cart quantities are normalized to 3 decimals at every mutation point (`computeLineAmounts`, `handleAddToCart` entry + merge) so float tails never reach state or the payload; money fields round to 2 decimals.

### Styling
- CSS files in `src/styles/css/` (Ticket, Sidebar) — legacy/global styles only; new UI uses MUI `sx`
- MUI `sx` style objects in `src/styles/js/` (FormStyles, PageHeader, PageContainer, StatsCards, SidebarStyles, DashboardStyles, SaleFormStyles, LoginPageStyles, etc.) — each exports a named `*Styles` const of plain `sx` objects (some are functions taking params)
- New/refactored UI is MUI-only (`sx`); per the AI guidelines, follow the `frontend-design` skill at `.claude/skills/frontend-design`
- **FontAwesome is loaded globally from `src/main.jsx`** (`import '@fortawesome/fontawesome-free/css/all.min.css'`). It used to be `@import`ed from the now-deleted `LoginPage.css`; four Bootstrap-era forms (`ProductForm`, `UserForm`, `CustomerForm`, `CustomerTypeForm`) still render `fas fa-save`/`fa-times`, so **do not remove that import** until those forms drop FontAwesome.
- **Login page (`/login`) is MUI-only** — `src/auth/pages/LoginPage.jsx` + `src/styles/js/LoginPageStyles.js` (`loginPageStyles`). Sober institutional look: neutral `#f4f6f4` ground with subtle pastel-green corner radial gradients, single 420px card, corporate green `#2e7d32` (hover `#1b5e20`) on a 48px-min submit button, organic (non-circular) `Avatar` holding the `Store` icon, and a show/hide password `IconButton` in the field's `endAdornment`. The old Bootstrap markup, the coral/turquoise gradient, and the emoji logo are gone. Auth logic is untouched — the page still consumes `handlerLogin` / `isLoginLoading` from `useAuth` (note the `handler*` spelling) and surfaces errors only through the hook's toasts.
...
- **Where stationId flows:** attached to every sale payload (`useSaleForm.handleSubmit` sets `stationId: getCachedStationId()`), passed as a `?stationId=` query param when fetching a ticket (`saleService.getTicketBySaleId(saleId, stationId)`) so the right terminal prints, and sent to `saleService.openDrawer(stationId)` (`POST /api/sales/open-drawer?stationId=...`, ADMIN/CAJERO only) to pop the cash drawer over WebSocket without a sale. A null/uncached stationId is tolerated when fetching tickets (param omitted); for open-drawer the backend requires it (400 if blank).
...
- Entity/DTO carries `imageUrl` — a **full public URL** (Cloudflare R2), so it is used directly as an `<img>`/`<Avatar>` source; **no** `VITE_API_BASE_URL` prefix.
